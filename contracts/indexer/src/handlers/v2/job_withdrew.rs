...
pub fn handle_job_withdrew(conn: &mut PgConnection, log: Log) -> Result<()> {
    ...
    let to = Address::from_word(log.topics()[3]).to_checksum(None);
    ...
    if is_usdc {
        handle_usdc_withdraw(conn, &id, &amount, block, idx, tx_hash)?;
    } else {
        handle_credits_withdraw(conn, &id, &amount, block, idx, tx_hash)?;
    }
...
fn handle_usdc_withdraw(
    ...
    info!(id, ?amount, "withdrew usdc from job");
    Ok(())
}

fn handle_credits_withdraw(
    conn: &mut PgConnection,
    id: &str,
    amount: &BigDecimal,
    block: u64,
    ...
    // target sql:
    // INSERT INTO transactions (block, idx, job, value, tx_type, is_usdc)
    // VALUES (block, idx, "<job>", "<value>", "withdraw", false);
    diesel::insert_into(transactions::table)
        .values((
            transactions::block.eq(block as i64),
            transactions::idx.eq(idx as i64),
    ...
    info!(id, ?amount, "withdrew credits from job");
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn test_usdc_withdraw_from_existing_job() -> Result<()> {
        // setup
        ...
        let log = Log {
            ...
            inner: alloy::primitives::Log {
                ...
                data: LogData::new(
                    vec![
                        event!("JobWithdrawn(bytes32,address,address,uint256)").into(),
    ...
    #[test]
    fn test_credits_withdraw_from_existing_job() -> Result<()> {
        // setup
        let mut db = TestDb::new();
        let conn = &mut db.conn;
    ...
    #[test]
    fn test_usdc_withdraw_from_non_existent_job() -> Result<()> {
        // setup
        let mut db = TestDb::new();
        let conn = &mut db.conn;
    ...
    #[test]
    fn test_credits_withdraw_from_non_existent_job() -> Result<()> {
        // setup
        let mut db = TestDb::new();
        let conn = &mut db.conn;
    ...
    #[test]
    fn test_usdc_withdraw_from_closed_job() -> Result<()> {
        // setup
        let mut db = TestDb::new();
        let conn = &mut db.conn;
    ...
    #[test]
    fn test_credits_withdraw_from_closed_job() -> Result<()> {
        // setup
        let mut db = TestDb::new();
        let conn = &mut db.conn;
