...
mod tests {
    ...
    #[test]
    fn test_usdc_deposit_into_existing_job() -> Result<()> {
        // setup
        ...
        let log = Log {
            ...
            inner: alloy::primitives::Log {
                ...
                data: LogData::new(
                    vec![
                        event!("JobDeposited(bytes32,address,address,uint256)").into(),
        ...
        assert_eq!(
            ...
            Ok(vec![
                (
                    ...
                    "0xaAaAaAaaAaAaAaaAaAAAAAAAAaaaAaAaAaaAaaAa".to_owned(),
                    Some(BigDecimal::from(1)),
                    Some(BigDecimal::from(25)),
                    Some(creation_now),
                    Some(creation_now),
                    false,
                    Some(BigDecimal::from(24)),
                    Some(BigDecimal::from(1)),
                ),
                (
                    "0x4444444444444444444444444444444444444444444444444444444444444444".to_owned(),
    ...
    #[test]
    fn test_credits_deposit_into_existing_job() -> Result<()> {
        // setup
        let mut db = TestDb::new();
        let conn = &mut db.conn;
        ...
        // we change the token address different from the mock usdc address
        let log = Log {
            block_hash: Some(keccak256!("some block").into()),
            block_number: Some(42),
            block_timestamp: None,
    ...
    #[test]
    fn test_usdc_deposit_into_non_existent_job() -> Result<()> {
        // setup
        let mut db = TestDb::new();
        let conn = &mut db.conn;
    ...
    #[test]
    fn test_credits_deposit_into_non_existent_job() -> Result<()> {
        // setup
        let mut db = TestDb::new();
        let conn = &mut db.conn;
    ...
    #[test]
    fn test_usdc_deposit_into_closed_job() -> Result<()> {
        // setup
        let mut db = TestDb::new();
        let conn = &mut db.conn;
    ...
    #[test]
    fn test_credits_deposit_into_closed_job() -> Result<()> {
        // setup
        let mut db = TestDb::new();
        let conn = &mut db.conn;
