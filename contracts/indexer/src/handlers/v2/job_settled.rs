...
pub fn handle_job_settled(conn: &mut PgConnection, log: Log) -> Result<()> {
    ...
    info!(id, ?timestamp, "settling job");
    ...
    let count = diesel::update(jobs::table)
        ...
        .set((jobs::last_settled.eq(&timestamp),))
        .execute(conn)
    ...
    fn test_settle_existing_job() -> Result<()> {
        ...
        let log = Log {
            ...
            inner: alloy::primitives::Log {
                ...
                data: LogData::new(
                    vec![
                        event!("JobSettled(bytes32,uint256)").into(),
                        "0x3333333333333333333333333333333333333333333333333333333333333333"
                            .parse()?,
                    ],
                    timestamp.abi_encode().into(),
                )
                .unwrap(),
            },
        };
    ...
    fn test_settle_closed_job() -> Result<()> {
        ...
        diesel::insert_into(jobs::table)
            .values((
                ...
                jobs::credits_balance.eq(BigDecimal::from(19)),
                jobs::last_settled.eq(&creation_now),
                jobs::created.eq(&creation_now),
