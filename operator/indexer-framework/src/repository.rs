...
use sqlx::{migrate::Migrator, postgres::PgPoolOptions, PgPool, Postgres, Row, Transaction};
use tracing::{debug, info};

use crate::schema::JobEventRecord;

const MIGRATION_PATH: &str = "../indexer-framework/migrations";
...
impl Repository {
    pub async fn new(db_url: String, start_block: Option<i64>) -> Result<Self> {
        ...
        if start_block.is_some() {
            let updated = Self::update_state_atomic(&pool, start_block.unwrap())
                .await
    ...
    pub async fn update_state_atomic(pool: &PgPool, block: i64) -> Result<u64> {
        let result = sqlx::query(
            r#"
            UPDATE indexer_state
            SET last_processed_block = $1, updated_at = now()
